...
export type SimulationRunOption = {
  simulation_run_id: string;
  source_system: string;
  season: number;
  week: number;
  ...
  players_simulated: number;
  status: string;
  has_role_shocks: boolean;
  has_point_in_time_shocks: boolean;
  started_at: string;
  completed_at?: string | null;
};

export type SimulationRunListResult = {
  rows: SimulationRunOption[];
  compatible_baseline_run_ids: string[];
};

export type UltimateLineupPortfolioExposureDelta = {
  player_master_id?: string | null;
  source_player_key?: string | null;
  player_name: string;
  team?: string | null;
  position: string;
  salary: number;
  baseline_exposure_count: number;
  baseline_exposure_rate: number;
  scenario_exposure_count: number;
  scenario_exposure_rate: number;
  exposure_rate_delta: number;
};

export type UltimateLineupPortfolioComparison = {
  simulation_run_id: string;
  baseline_simulation_run_id?: string | null;
  shared_candidate_lineups: number;
  baseline_output_lineups: number;
  scenario_output_lineups: number;
  lineup_overlap_count: number;
  lineup_overlap_rate: number;
  baseline_portfolio_baseline_projected_blend: number;
  baseline_portfolio_scenario_projected_blend: number;
  scenario_portfolio_scenario_projected_blend: number;
  projected_blend_reoptimization_lift: number;
  baseline_portfolio_scenario_objective_score: number;
  scenario_portfolio_scenario_objective_score: number;
  objective_reoptimization_lift: number;
  exposure_changes: UltimateLineupPortfolioExposureDelta[];
};

export type UltimateLineupResult = {
  source_system: string;
  season: number;
  week: number;
  slate: string;
  simulation_run_id?: string | null;
  simulation_outcomes_loaded: number;
  simulation_projection_overrides: number;
  baseline_simulation_run_id?: string | null;
  baseline_simulation_outcomes_loaded: number;
  baseline_simulation_projection_overrides: number;
  portfolio_comparison?: UltimateLineupPortfolioComparison | null;
  contest_objective: "balanced" | "cash" | "gpp";
  candidate_lineups_requested: number;
  generated_candidate_lineups: number;
  output_lineups: number;
};

export type UltimateLineupRun = {
  ultimate_lineup_run_id: string;
  ...
  completed_at?: string | null;
  result?: UltimateLineupResult | null;
};

export type UltimateLineupRequestPayload = {
  source_system: "draftkings" | "fanduel";
  season: number;
  week: number;
  slate: string;
  simulation_run_id: string;
  baseline_simulation_run_id?: string;
  contest_objective: "balanced" | "cash" | "gpp";
  candidate_lineups: number;
  output_lineups: number;
...
export function fetchSimulationRuns(params: {
  ...
  limit?: number;
}): Promise<SimulationRunListResult> {
  ...
  if (params.scenario_run_id) {
    search.set("scenario_run_id", params.scenario_run_id);
  }
  return getJson(`/simulate/runs?${search.toString()}`);
}

export function generateUltimateLineups(
  payload: UltimateLineupRequestPayload
): Promise<UltimateLineupResult> {
  return postJson("/lineups/ultimate", payload);
}

export function createUltimateLineupRun(payload: {
  idempotency_key: string;
  request: UltimateLineupRequestPayload;
}): Promise<{ created: boolean; run: UltimateLineupRun }> {
  return postJson("/lineups/ultimate-runs", payload);
}

export function fetchUltimateLineupRun(
  ultimateLineupRunId: string
): Promise<UltimateLineupRun> {
  return getJson(`/lineups/ultimate-runs/${encodeURIComponent(ultimateLineupRunId)}`);
}

export function retryUltimateLineupRun(
  ultimateLineupRunId: string
): Promise<UltimateLineupRun> {
  return postJson(
    `/lineups/ultimate-runs/${encodeURIComponent(ultimateLineupRunId)}/retry`,
    {}
  );
}
