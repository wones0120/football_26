...
function singleEntryReportHtml(lineups: ReportPlayer[][]): string {
  ...
  const contestHtml = rankedContests.length ? `<h3>Contest selection</h3><p>${escapeHtml(contestSelection.mode)} · ${number(contestSelection.selected_count, 0)} of ${number(contestSelection.available_count, 0)} contests · $${number(contestSelection.total_entry_fees, 2)} total entry fees. ${escapeHtml(contestSelection.note)}</p><table><thead><tr><th>Selected</th><th>Contest</th><th>Score</th><th>Entry</th><th>Field</th><th>Paid</th><th>Rake</th><th>Potential overlay</th></tr></thead><tbody>${rankedContests.map((row) => { const parts = (row.contest_score_components ?? {}) as ReportPlayer; return `<tr><td>${selectedIds.includes(String(row.contest_id)) ? "Yes" : "No"}</td><td>${escapeHtml(row.name)}</td><td>${number(row.contest_score, 3)}</td><td>$${number(row.entry_fee, 2)}</td><td>${number(row.capacity, 0)}</td><td>${number(row.paid_places, 0)}</td><td>${number((1 - Number(parts.rake_score)) * 100, 1)}%</td><td>${number(Number(parts.potential_overlay) * 100, 1)}%</td></tr>`; }).join("")}</tbody></table>` : "";
  ...
  const candidateRows = unique.map((row) => {
    ...
    return `<tr><td>${number(row.rank, 0)}</td><td>${escapeHtml(row.captain)}</td><td>${number(row.mean, 1)}</td><td>${number(row.p90, 1)}</td><td>${number(row.relative_chalk, 1)}</td><td>${escapeHtml(script.label)}</td><td>${number(row.shared_players_with_a, 0)}/6</td><td>${number(row.overlap_percentage_vs_a, 1)}%</td><td>${number(row.jaccard_similarity_vs_a, 3)}</td><td>${number(row.quality_loss_vs_a, 3)}</td><td>${number(row.diversification_benefit_vs_a, 3)}</td><td>${escapeHtml(players.map((player) => `${player.slot} ${player.name}`).join(", "))}</td></tr>`;
  }).join("");
  return `<section class="lineup"><h2>Recommended portfolio: ${escapeHtml(report.recommended_structure ?? "A")}</h2>
    ${contestHtml}
    <p>Heuristic selection. Payout probabilities require game, field, and contest simulation. P90 is the sum of player P90 projections.</p>
    <p>Compared ${number(report.evaluated_portfolio_count, 0)} candidate combinations with repetition; the table shows the strongest in each structure.</p>
    ...
    ${comparisonRows.length ? `<table><thead><tr><th>Structure</th><th>Candidate ranks</th><th>Quality loss</th><th>Diversification credit</th><th>Delta vs A / A / A</th></tr></thead><tbody>${comparisonHtml}</tbody></table>` : ""}
    <h3>Closest construction alternatives</h3><p>Material player change: at least ${number(report.material_player_change_minimum, 0)} of six players.</p>
