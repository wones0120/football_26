...
| ID | Status | Work | Dependencies | Acceptance check | Recommended model |
| --- | --- | --- | --- | --- | --- |
| DT-501 | Done | Added explicit `classic_gpp_baseline_v1` and `classic_gpp_slate_aware_v1` contracts. The baseline remains the default; selecting the advanced version executes it against the exact live lineage-scoped pool with exclusions, DT-502 evidence, exposure, single-TE, and offense-versus-DST controls intact. Strategy/config/portfolio/stack explanations persist and reload, API/UI results identify the engine, and validation failures never silently fall back across strategies. | DT-104, DT-105 | The selected strategy is executed, validated, persisted, and visible in results. | GPT-5.6 Sol High |
