...
| ID | Status | Work | Dependencies | Acceptance check | Recommended model |
| --- | --- | --- | --- | --- | --- |
| DT-301 | Done | Replace the active all-position/global-mean and constant-sigma output with position- and role-aware point/distribution estimates. The active v3 path removes historically confirmed nonparticipants while retaining real zero-point games, adds explicit QB/RB/WR/TE/DST indicators, blends 80% model output with 20% prior-only player history, and builds strictly chronological out-of-fold residuals. A lagged three-game snap share supports receiver roles; monotonic P10/P25/P50/P75/P90 quantiles remain hierarchical by position and role. Exact filters, weights, features, position baselines, calibration diagnostics, and promotion checks persist in model lineage and surface through the API and Model Workbench. | DT-101, DT-102 | Walk-forward point MAE and P10/P25/P50/P75/P90 coverage are measured by position and role. | GPT-5.6 Sol XHigh |
...
| ID | Status | Work | Dependencies | Acceptance check | Recommended model |
| --- | --- | --- | --- | --- | --- |
| DT-501 | Done | Added historical `classic_gpp_baseline_v1` / `classic_gpp_slate_aware_v1` contracts and the current explicit `classic_head_to_head_v1` / `classic_large_gpp_v1` contest strategies. Both current modes share the exact live lineage-scoped eligibility pool, then diverge into broad mean-dominant H2H selection or ceiling/correlation/diversification-led GPP portfolio generation. Strategy/config/portfolio/realized-stack explanations persist and reload; UI/API results show eligible/included/excluded counts and concrete exclusion reasons, and validation failures never silently switch engines. | DT-104, DT-105 | The selected strategy is executed, validated, persisted, and visible in results. | GPT-5.6 Sol High |
...
| ID | Status | Work | Dependencies | Acceptance check | Recommended model |
| --- | --- | --- | --- | --- | --- |
| DT-601 | Done | Added explicit `showdown_cash_baseline_v1` and `showdown_gpp_baseline_v1` contracts around the basic P90 captain ILP. Operations and weekly runs send the canonical mode-specific ID; incompatible objective/strategy combinations fail explicitly. Independent validation enforces one CPT, five FLEX, canonical identity, salary, team, exposure, and uniqueness rules. Normalized slot indexes and roles persist with successful lineups, while completed or failed runs reload format, objective, strategy/config, status/message, projection and rule IDs, cutoff, and results after restart. Distinct stability/payout objectives remain DT-605 work. | DT-104, DT-105 | Showdown cash and GPP runs are distinct, persisted, and reloadable. | GPT-5.6 Sol High |
...
| DT-606 | Done | Added canonical starter-QB eligibility and `showdown_gpp_captain_informed_v1`. The UI uses the validated 0.35 captain prior, preserves the basic GPP baseline for comparisons, records OOD fallback to the historical captain mix, and displays both roster slot and natural position. | DT-601 | Backup QBs cannot enter the pool; ambiguous evidence fails closed; captain and starter lineage persists. | GPT-5.6 Sol High |
...
| DT-704 | Done | Added versioned, deterministic value-of-information questions from immutable DT-703 artifacts. At most five high-priority model/human disagreements or wide player distributions are shown; every answer and no-change response stores its exact evidence and inert proposed modifier. | DT-703 | Questions are triggered by defined value-of-information rules and every answer is recorded. | GPT-5.6 Sol XHigh |
| DT-705 | Done | Added time-safe belief-thesis and intervention scoring to immutable post-slate reports. Exact approved/rejected belief previews and LEARN-002 answers are compared with their stored counterfactuals, grouped by scope and confidence, and late or incomplete evidence remains unscored. | DT-703 | Reports show when human intervention helped, hurt, or had no measurable effect. | GPT-5.6 Sol XHigh |
...
1. Gather prospective Week 2 beliefs, LEARN-002 answers, and OPT-007 controls so DT-705 has time-safe evidence.
